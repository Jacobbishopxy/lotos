...
- [ ] **Complete reliable worker logging transport switch** — TP-021 selected the DEALER/ROUTER architecture, TP-022 added the `LogEvent`/`LogBatch`/`LogAck` protocol plus `LogIngestConfig`, and TP-023 added the broker LogIngest state, JSONL journal, bounded query cache, ROUTER ACK loop, and `/logs/...` query routes. Remaining work is to add the worker logging DEALER with bounded retry/drop handling, enable split logging endpoints in runtime configs, add journal restart recovery/retention, and rewire InfoStorage/TaskSchedule smoke paths away from the current PUB/SUB compatibility path.
