...
- [x] **TP-030 decision:** keep direct polling in `LBW.socketLoop` for worker backend DEALER + internal PAIR transport. EventLoop migration was deferred due callback-in-loop pressure and shutdown/error-mode complexity (`stopped-loop`/`ETERM`) without measured latency gains; revisit only with a dedicated lifecycle redesign plus benchmark proof.
