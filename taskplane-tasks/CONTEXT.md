...
- [ ] **Implement LogIngest reliable worker logging** — TP-021 selected the DEALER/ROUTER architecture and TP-022 added the `LogEvent`/`LogBatch`/`LogAck` protocol plus `LogIngestConfig` compatibility surface. Remaining work is to start the broker LogIngest service, add the worker logging DEALER with bounded retry/drop handling, persist/compact the journal, maintain the bounded read cache, and rewire InfoStorage away from the current PUB/SUB compatibility path.
