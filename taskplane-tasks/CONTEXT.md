...
- [ ] **Evaluate remaining zmqx EventLoop/socket-ownership follow-ups** — TP-027 pinned `zmqx` to `v0.1.1.1` and established a green direct-API baseline; TP-028 removed the worker executor's fixed 10-second empty-queue sleep with a bounded STM wake-on-enqueue signal, added wake/count regression coverage, and removed the broker socket-layer `> poll` stdout spam. Remaining work should stay focused on optional `Zmqx.EventLoop` socket ownership/polling or other runtime-loop hot paths without changing established multipart frame ordering unless covered by a dedicated follow-up test plan.
