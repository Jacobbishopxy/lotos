...
- [ ] **Implement retry delay semantics** — `taskRetryInterval` remains schema-only; failed tasks with retries are requeued immediately rather than delayed by the interval (discovered during TP-012).
- [ ] **Keep retry-disposition helper internal** — `failedTaskDisposition` is currently exported through `Lotos.Zmq` to support bounded tests; consider moving it behind an internal testable module if the public API is tightened later (discovered during TP-012).
