...
- [ ] **Keep retry-disposition helpers internal** — `failedTaskDisposition` plus TP-015 retry-delay helpers (`RetryTask`, `mkRetryTask`, `retryTaskEligible`, `partitionRetryTasks`) are currently exported through `Lotos.Zmq` to support bounded tests and the public `TaskSchedulerData` failed-queue type; consider moving them behind an internal testable module if the public API is tightened later (discovered during TP-012, updated during TP-015).
