    # Task: TP-054 - Explicit protocol versioning plan
    ...
    **Score:** Blast radius: 1, Pattern novelty: 1, Security: 0, Reversibility: 1

    ## Canonical Task Folder

    ```
    /home/xiey/Code/lotos/taskplane-tasks/TP-054-explicit-protocol-versioning-plan/
    ...
    Turn the TP-046 compatibility policy into a concrete plan for future incompatible ZMQ payload changes: when to add discriminators/endpoints/versioned payloads, how to test migration, and what not to silently widen.

    ## Dependencies

    - **Task:** TP-053 must be complete
...
- `lotos/test/ZmqLogProtocolConfig.hs`
- `taskplane-tasks/CONTEXT.md`
    ...
    - [ ] Inventory current discriminators/routes and golden fixture coverage
- [ ] Write a versioning decision matrix for append-only vs incompatible changes
- [ ] Add lightweight tests or test comments only if current fixtures need clearer versioning hooks
- [ ] Cross-link README/mdBook guidance for future maintainers
...
- [ ] Run `make book-build`
    ...
    **Must Update:**
    - `docs/book/lotos/src/protocol-compatibility.md` — update if affected
- `docs/book/lotos/src/zmq-protocol.md` — update if affected
...
- `README.md` — update if affected
    ...
    **Check If Affected:**
    ...
    Commits happen at **step boundaries**. All commits for this task MUST include `TP-054` for traceability.

    ## Do NOT

    - Do not introduce protocol-wide version tags prematurely unless tests prove an immediate need
- Do not weaken exact decoder failures
- Do not reorder multipart frames
    - Skip verification
