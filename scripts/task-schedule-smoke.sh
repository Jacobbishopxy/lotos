...
TASK_JSON="$EVIDENCE_DIR/task-demo.json"
...
{
  "taskID": null,
  "taskContent": "TP-005 smoke run $RUN_ID",
  "taskRetry": 0,
  "taskRetryInterval": 0,
...
JSON
  log "wrote task JSON to $TASK_JSON"
}

run_client() {
  log "submitting task through ts-client"
  if have_cmd timeout; then
    timeout "$CLIENT_TIMEOUT_SEC" cabal run TaskSchedule:exe:ts-client -- "$CLIENT_CONFIG" "$TASK_JSON" >"$CLIENT_STDIO_LOG" 2>&1
  else
    cabal run TaskSchedule:exe:ts-client -- "$CLIENT_CONFIG" "$TASK_JSON" >"$CLIENT_STDIO_LOG" 2>&1
  fi
}
