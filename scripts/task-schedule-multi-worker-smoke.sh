...
name = "TP-017 multi-worker smoke $label"
description = "Fresh multi-worker smoke task generated by scripts/task-schedule-multi-worker-smoke.sh."
labels = ["smoke", "multi-worker"]
...
TOML
    ...
    TASK_TOMLS+=("$task_toml")
    MARKER_FILES+=("$marker_file")
    log "wrote client config $client_config and task TOML $task_toml"
  done
}
...
submit_clients() {
  ...
  for ((idx=0; idx<${#TASK_TOMLS[@]}; idx++)); do
    if have_cmd timeout; then
      start_tracked "client-$((idx + 1))" "${CLIENT_STDIO_LOGS[$idx]}" timeout "$CLIENT_TIMEOUT_SEC" cabal run TaskSchedule:exe:ts-client -- "${CLIENT_CONFIGS[$idx]}" "${TASK_TOMLS[$idx]}"
    else
      start_tracked "client-$((idx + 1))" "${CLIENT_STDIO_LOGS[$idx]}" cabal run TaskSchedule:exe:ts-client -- "${CLIENT_CONFIGS[$idx]}" "${TASK_TOMLS[$idx]}"
    fi
...
wait_for_worker_execution_evidence() {
  ...
  while [ "$SECONDS" -le "$deadline" ]; do
    ...
    for ((idx=0; idx<${#WORKER_IDS[@]}; idx++)); do
      if ! grep -F "$RUN_ID" "$LOG_INGEST_JOURNAL_PATH" 2>/dev/null | grep -F "\"workerId\":\"${WORKER_IDS[$idx]}\"" >/dev/null 2>&1; then
        all_found=0
      fi
    done
    if [ "$all_found" -eq 1 ]; then
      log "current-run task evidence found in LogIngest journal for every worker"
      return 0
    fi
    sleep 1
  done
