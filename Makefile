...
.PHONY: help tree clean update build ci-build ci-test ci-docs ci-check book-build book-serve docs-build docs-serve dashboard-install dashboard-build dashboard-dev dashboard-preview task-schedule-build-server task-schedule-build-worker task-schedule-build-client task-schedule-build-client-bridge task-schedule-build-all release-clean release-server release-worker release-client release-client-bridge release-task-schedule release-manifest task-schedule-server task-schedule-broker task-schedule-worker task-schedule-client-bridge task-schedule-submit task-submit task-validate task-template example-minimal smoke-single smoke-multi smoke-dashboard-bridge smoke-dashboard-browser hie
...
dashboard-dev:
	DASHBOARD_HOST="$(DASHBOARD_HOST)" DASHBOARD_API_TARGET="$(DASHBOARD_API_TARGET)" DASHBOARD_BRIDGE_TARGET="$(DASHBOARD_BRIDGE_TARGET)" VITE_TASKSCHEDULE_API_TARGET="$(DASHBOARD_API_TARGET)" VITE_TASKSCHEDULE_API_BASE="$(DASHBOARD_API_BASE)" VITE_TASKSCHEDULE_API_ROOT="$(DASHBOARD_API_ROOT)" VITE_TASKSCHEDULE_API_TIMEOUT_MS="$(DASHBOARD_API_TIMEOUT_MS)" VITE_TASKSCHEDULE_BRIDGE_BASE="$(DASHBOARD_BRIDGE_BASE)" VITE_TASKSCHEDULE_BRIDGE_PATH="$(DASHBOARD_BRIDGE_PATH)" VITE_TASKSCHEDULE_BRIDGE_TIMEOUT_MS="$(DASHBOARD_BRIDGE_TIMEOUT_MS)" npm --prefix $(DASHBOARD_DIR) run dev
...
release-client-bridge: task-schedule-build-client-bridge
	@mkdir -p $(TASKSCHEDULE_RELEASE_DIR)/bin $(TASKSCHEDULE_RELEASE_DIR)/config
	...
	install -m 644 $(TASKSCHEDULE_CLIENT_BRIDGE_CONFIG) $(TASKSCHEDULE_RELEASE_DIR)/config/client-bridge.json
	$(MAKE) release-manifest
...
task-schedule-client-bridge:
	cabal run TaskSchedule:exe:ts-client-bridge -- $(TASKSCHEDULE_CLIENT_BRIDGE_CONFIG)
