...
Root Make targets pass Vite environment variables for the dev host, API root, proxy target, direct API base, and request timeout:
...
- `DASHBOARD_HOST` defaults to `0.0.0.0` for shareable local development.
...
- If the probe works but the browser fetch fails in dev mode, restart `make dashboard-dev` after changing `DASHBOARD_HOST`, `DASHBOARD_API_TARGET`, or `DASHBOARD_API_ROOT`; Vite reads server/proxy settings at startup.
