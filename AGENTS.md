...
This repository is a Haskell/Cabal workspace for a ZeroMQ-backed task load-balancer library plus a `TaskSchedule` demo application. Use these notes before the generic OMX guidance below.

## Source map

- `cabal.project` defines the workspace packages:
  - `lotos/lotos.cabal` — reusable core library.
  - `applications/TaskSchedule/TaskSchedule.cabal` — example app package.
- `lotos/src/Lotos/Zmq.hs` is the public facade for the ZMQ load-balancer API.
- `lotos/src/Lotos/Zmq/Adt.hs` contains protocol types, task/status types, and `ToZmq`/`FromZmq` multipart serialization.
- `lotos/src/Lotos/Zmq/LBS*.hs` implements the load-balancer server, socket layer, task processor, and HTTP info storage.
- `lotos/src/Lotos/Zmq/LBW.hs` implements worker services and the `TaskAcceptor`/`StatusReporter` extension points.
- `lotos/src/Lotos/Zmq/LBC.hs` implements the client request service.
- `lotos/src/Lotos/Proc/ConcExecutor.hs` runs shell commands concurrently with callbacks, logs, and timeout handling.
- `lotos/src/Lotos/TSD/` contains STM-backed queue/map/ring-buffer helpers.
- `applications/TaskSchedule/src/` binds the framework to shell-command tasks and load-based scheduling.
- `applications/TaskSchedule/app/` contains the `ts-server`, `ts-worker`, and placeholder `ts-client` executables.
...
- Preserve exact `ToZmq`/`FromZmq` frame ordering. Router, worker, and client code rely on matching multipart message shapes.
- Maintain the task-ID invariant: tasks should have UUIDs before scheduling/worker execution; `unsafeGetTaskID` assumes this.
- Be careful around hardcoded demo addresses. The server uses frontend `tcp://127.0.0.1:5555` and backend `tcp://127.0.0.1:5556`; the worker executable currently appears to use these inversely.
