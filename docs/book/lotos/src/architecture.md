...
Reservations are conservative: non-terminal task statuses such as `TaskPending` and `TaskProcessing` keep consuming capacity while their broker reservation is still active, and heartbeat/liveness updates do not blindly clear the broker-known slot if the snapshot may be stale. Capacity is released when heartbeat reconciliation safely proves the worker status already accounts for the slot, when terminal task status (`TaskSucceed`/`TaskFailed`) arrives, or when stale-worker recovery proves the worker is no longer holding that slot. Late or duplicate non-terminal task-status updates refresh active reservations only; they do not recreate a reservation that safe heartbeat reconciliation already removed.
